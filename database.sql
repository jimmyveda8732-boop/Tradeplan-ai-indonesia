...
drop policy if exists "Users can view their own trading journal" on public.trading_journal;
create policy "Users can view their own trading journal"
  on public.trading_journal
  for select
  using (auth.uid() = user_id);

drop policy if exists "Users can insert their own trading journal" on public.trading_journal;
create policy "Users can insert their own trading journal"
  on public.trading_journal
  for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update their own trading journal" on public.trading_journal;
create policy "Users can update their own trading journal"
  on public.trading_journal
  for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "Users can delete their own trading journal" on public.trading_journal;
create policy "Users can delete their own trading journal"
  on public.trading_journal
  for delete
  using (auth.uid() = user_id);
...
drop trigger if exists trading_journal_set_updated_at on public.trading_journal;
create trigger trading_journal_set_updated_at
